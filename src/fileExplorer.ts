...
export class FileSystemProvider implements vscode.TreeDataProvider<Entry>, vscode.FileSystemProvider {
	...
	watch(uri: vscode.Uri, options: { recursive: boolean; excludes: string[]; }): vscode.Disposable {
		const watcher = fs.watch(uri.fsPath, { recursive: options.recursive }, async (event: string, filename: string | Buffer) => {
			if (!filename) {
				console.log("filename not varid");
				return;
			}
			const filepath = path.join(uri.fsPath, _.normalizeNFC(filename.toString()));

			if (filepath.match("\.tmp$")){
				return ;
			}
			if (filepath.match("\\.mPDF.*\.pdf$")){
				vscode.commands.executeCommand("fileOrder.merge").then(undefined, err => {
	...
	getTreeItem(element: Entry): vscode.TreeItem {
		...
		if (element.type === vscode.FileType.File) {
			// treeItem.command = { command: 'fileExplorer.selectFile', title: "Select File", arguments: [element.uri], };
...
export class FileExplorer {
	...
	private openResource(resource: vscode.Uri): void {
		vscode.commands.executeCommand("fileOrder.add", resource);
	}
}
